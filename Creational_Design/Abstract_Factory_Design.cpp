...
// this code becomes more dirty and violated open closed principle by adding more if else conditions for each vehicle
...
class Honda: public Vehicle{
    public:
        void start() override{
            cout<<"Honda is Starting"<<endl;
        }
        void stop() override{
            cout<<"Honda is stoping"<<endl;
        }
};
...
int main(){
    ...
    while(T--){
            ...
            Vehicle *bmw = factory2->createVehicle();
            bmw->start();
            bmw->stop();
    }
}